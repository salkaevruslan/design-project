...
async def create_user_task(request: UserTaskCreationRequest, current_user: User = Depends(get_current_user),
    ...
    return tasks_service.create_user_task(db, current_user, request.type, request.name, request.description,
                                          request.priority, request.start_time)
...
async def create_group_task(request: GroupTaskCreationRequest, current_user: User = Depends(get_current_user),
                            db: Session = Depends(get_database)):
    return tasks_service.create_group_task(db, current_user, request.group_id, request.type, request.name,
                                           request.description, request.priority, request.start_time)


@router.post("/all/list")
async def all_user_tasks(request: TaskFilterRequest, current_user: User = Depends(get_current_user),
                         db: Session = Depends(get_database)):
    return tasks_service.get_all_tasks(db, current_user, request.period_start,
                                       request.period_end, request.type, request.priority)
...
async def my_personal_tasks(request: TaskFilterRequest, current_user: User = Depends(get_current_user),
                            db: Session = Depends(get_database)):
    return tasks_service.get_personal_tasks(db, current_user, request.period_start,
                                            request.period_end, request.type, request.priority)
...
async def my_group_tasks(request: GroupTaskFilterRequest, current_user: User = Depends(get_current_user),
                         db: Session = Depends(get_database)):
    return tasks_service.get_group_tasks(db, current_user, request.group_id, request.period_start,
                                         request.period_end, request.type, request.priority)
...
async def suggest_group_task(request: GroupTaskCreationRequest, current_user: User = Depends(get_current_user),
                             db: Session = Depends(get_database)):
    return tasks_service.create_group_task_suggestion(db, current_user, request.group_id, request.type, request.name,
                                                      request.description, request.priority, request.start_time)
...
async def get_my_group_task_suggestion(request: GroupTaskFilterRequest, current_user: User = Depends(get_current_user),
                                       db: Session = Depends(get_database)):
    return tasks_service.get_my_task_suggestions_to_group(db, current_user, request.group_id, request.period_start,
                                                          request.period_end, request.type, request.priority)
...
async def get_all_group_task_suggestion(request: GroupTaskFilterRequest, current_user: User = Depends(get_current_user),
                                        db: Session = Depends(get_database)):
    return tasks_service.get_all_task_suggestions_to_group(db, current_user, request.group_id, request.period_start,
                                                           request.period_end, request.type, request.priority)
...
async def update_user_task(request: TaskUpdateRequest, current_user: User = Depends(get_current_user),
                           db: Session = Depends(get_database)):
    tasks_service.update_user_task(db, current_user, request.task_id, request.status, request.type, request.description,
                                   request.priority, request.start_time)
...
async def update_group_task(request: TaskUpdateRequest, current_user: User = Depends(get_current_user),
                            db: Session = Depends(get_database)):
    tasks_service.update_group_task(db, current_user, request.task_id, request.status, request.type,
                                    request.description,
                                    request.priority, request.start_time)
...
async def update_suggested_task(request: TaskUpdateRequest, current_user: User = Depends(get_current_user),
                                db: Session = Depends(get_database)):
    tasks_service.update_suggested_task(db, current_user, request.task_id, request.status, request.type,
                                        request.description,
                                        request.priority, request.start_time)
